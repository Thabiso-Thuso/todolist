...
export function getTasks(sort : string){
    return db.prepare(`SELECT * FROM tasks ORDER BY ${sort}`).all();
}

export function createTask(title : string, description : string, dueDate : string, topic: string){
    const stmt = db.prepare("INSERT INTO tasks (title, description, dueDate, topic) VALUES (?, ?, ?, ?)");
